...
function select_profile_type(){
    printf "\n"
    COLUMNS=12
    echo -e "\x1B[1mPlease select the deployment profile (default: small).  Refer to the documentation in IBM Business Automation Workflow on containers Knowledge Center for details on profile.\x1B[0m"
...
\\${nl}" ${DB_SERVER_INFO_PROPERTY_FILE}
    ...
    INFO "Creating property file for user profile required by CP4BA"
    ...
    echo "## false: All CP4A pods will have access any external systems with no restriction." >> ${USER_PROFILE_PROPERTY_FILE}
    ...
    echo -e  "  - Properties for database server used by BAW on containers, such as DATABASE_SERVERNAME/DATABASE_PORT/DATABASE_SSL_ENABLE.\n"
    ...
    echo -e  "  - Properties for the LDAP server that is used by the BAW on containers, such as LDAP_SERVER/LDAP_PORT/LDAP_BASE_DN/LDAP_BIND_DN/LDAP_BIND_DN_PASSWORD.\n"
    if [[ $SET_EXT_LDAP == "Yes" ]]; then
    ...
    echo -e  "  - Properties for the global value used by the BAW on containers deployment, such as \"sc_deployment_license\".\n"
    echo -e  "  - properties for the value used by each component of BAW on containers, such as <APPLOGIN_USER>/<APPLOGIN_PASSWORD>\n"
}
...
function create_db_script(){
   ...
   # Generate DB SQL for Objectstore
    ...
    tips ""
    msgB "* The DB SQL statement files were created under directory ${DB_SCRIPT_FOLDER}, you could modify or use default setting to create database.\n(NOTES: PLEASE DO NOT CHANGE DBNAME/DBUSER/DBPASSWORD DIRECTLY in DB SQL statement files. PLEASE CHANGE THEM IN PROPERTY FILES IF NEEDED)"
...
function select_enable_event_emitter() {
    printf "\n"
    while true; do
        printf "\x1B[1mDo you want to enable Case Event Emitter with this BAW deployment? (Yes/No, default: No): "
        read -rp "" ans
        case "$ans" in
        "y"|"Y"|"yes"|"Yes"|"YES")
...
function get_deployment_hostname_suffix() {
    ...
    do
        printf "\x1B[1mPlease enter the deployment hostname suffix: \x1B[0m"
...
function get_db_server_list(){
    ...
    while true; do
        printf "\n"
        printf "\x1B[1mEnter the alias name(s) for database server(s)/instance(s) to be used by the BAW on containers.\x1B[0m\n"
...
function select_restricted_internet_access(){
    ...
    echo ""
    while true; do
        printf "\x1B[1mDo you want to restrict network egress to unknown external destination for this BAW deployment?\x1B[0m ${YELLOW_TEXT}\x1B[1;31m(NOTE: BAW $CP4BA_RELEASE_BASE prevents all network egress to unknown destinations by default. You can either (1) enable all egress or (2) accept the new default and create network policies to allow your specific communication targets as documented in the knowledge center.)\x1B[0m${RESET_TEXT} (Yes/No, default: Yes): "
        read -rp "" ans
        case "$ans" in
        "y"|"Y"|"yes"|"Yes"|"YES"|"")
