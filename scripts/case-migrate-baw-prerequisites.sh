...
LICENSE_CP4A_URL="https://www14.software.ibm.com/cgi-bin/weblap/lap.pl?li_formnum=L-BHEG-VY4FQR"
...
function show_help() {
    ...
    echo "Options:"
    echo "  -h  Display help"
    echo "  -m  The valid mode type: [property] or [generate] or [validate] or [generate-cr]"
    echo "      STEP1: Run with [property] mode: create user property file (DB/LDAP property file) with default value (database name/user)."
    echo "      STEP2: Modify DB/LDAP/User property file as your requirement."
    echo "      STEP3: Run with [generate] mode: generate DB SQL statement file/YAML template for secret automatically based on the value in property file."
    echo "      STEP4: Create database and secret manually based on modified DB SQL statement file/YAML template for secret ."
    echo "      STEP5: Run with [validate] mode: check the database and secret is created or not before deployment BAW."
    echo "      STEP6: Run with [generate-cr] mode: Generate the Business Automation Workflow custom resource based on the property files."
}
...
function create_case_migration_property_file() {
    ...
    do
        ...
        ${YQ_CMD} w -i ${CASE_MIGRATION_PROPERTY_FILE} "case.tos_list[$i].is_default" --style=double ${MIG_PROP_TEMP}
    done
    ...
    do
        ...
        if [ -z "$secret_name_tmp" ]; then
            ...
            if [ "$secret_exists" -ne 2 ] ; then
                error "Not found secret \"$secret_name_tmp\" in Kubernetes cluster! please create it firstly before deployment CP4BA"
...
function case_migration_apply_pattern_cr() {
    ...
    do
        ...
        case_migration_replace "case.tos_list[$j].is_default" "spec.baw_configuration[0].case.tos_list[$j].is_default" "q"
...
if [[ $RUNTIME_MODE == "property" ]]; then
    ...
    while [ "$valid_int" = false ]; do
        printf "\x1B[1mPlease provide Number of Target Object Stores \x1B[0m \x1B[33m[Minimum 1] \x1B[0m :"
        read -rp "" TOS_NUM
        ...
        then 
            ...
            echo -e "  - Please update the property file ${CASE_MIGRATION_PROPERTY_FILE} and rerun the migration with generate"
        else 
            echo -e "\x1B[1;31mPlease provide a valid input for Number of Target Object Stores\x1B[0m"
...
if [[ $RUNTIME_MODE == "generate" ]]; then
   if [ -e $CASE_MIGRATION_PROPERTY_FILE ] ; 
        then  
            ...
            if [ $value_empty -ne 0 ] ; 
            then
                error "Found invalid value(s) \"<Required>\" in property file \"${CASE_MIGRATION_PROPERTY_FILE}\", please input the correct value and rerun"
                exit 1
            fi
            #Validate the property file for Syntax errors
            if ! ${YQ_CMD} validate $CASE_MIGRATION_PROPERTY_FILE ;
            then 
                error "Invalid Property File Syntax (YAML): \"${CASE_MIGRATION_PROPERTY_FILE}\", please correct the synatx and rerun"
                exit 1
            fi 
        #Import & exeute baw-prerequisites
        source ${CUR_DIR}/baw-prerequisites.sh
        ...
        error "Migration property file : \"${CASE_MIGRATION_PROPERTY_FILE}\" not found , please rerun the migration with -m property"
        exit 1
    fi
fi
...
if [[ $RUNTIME_MODE == "generate-cr" ]]; then
    ...
    then  
        ...
        if [ $value_empty -ne 0 ] ; 
        then
            error "Found invalid value(s) \"<Required>\" in property file \"${CASE_MIGRATION_PROPERTY_FILE}\", please input the correct value and rerun the migration"
            exit 1
        fi
        #Validate the property file for Syntax errors
        if ! ${YQ_CMD} validate $CASE_MIGRATION_PROPERTY_FILE ;
        then 
            error "Invalid Property File Syntax (YAML): \"${CASE_MIGRATION_PROPERTY_FILE}\", please correct the synatx and rerun the migration"
            exit 1
        fi 
    else 
        error "Migration Property File not Found: \"${CASE_MIGRATION_PROPERTY_FILE}\", Please rerun the case-migrate-baw-prerequisites.sh to create the file"   
        exit 1
    fi
            
        
    ##########################################
    # Prateek Function to do changes on cr based on migration requirement
