...
function validate_case_migrate_prerequisites(){
    ...
    if [[ " ${pattern_cr_arr[@]}" =~ "document_processing" || " ${pattern_cr_arr[@]}" =~ "application" || " ${pattern_cr_arr[@]}" =~ "workflow-runtime" || " ${pattern_cr_arr[@]}" =~ "workstreams" ]]; then
        if [[ $DB_TYPE != "oracle" ]]; then
...
if [[ $RUNTIME_MODE == "property" ]]; then
    ...
    while [ "$valid_int" = false ]; do
        printf "\x1B[1mPlease provide Number of Target Object Stores[Minimum 1] : \x1B[0m"
        read -rp "" TOS_NUM
        #if [[ $TOS_NUM =~ ^[1-9]+$ ]];
        if [[ $TOS_NUM -ge 1 ]];
        then 
            ...
            echo -e "Please provide a valid input for Number of Target Object Stores"
        fi
    done 
