...
function case_migration_apply_pattern_cr() {
    ...
    then 
        ...
        do
            ...
            case_migration_replace "case.tos_list[$j].is_default" "spec.workflow_authoring_configuration.case.tos_list[$j].is_default" "q"
        done
...
then
    #printf "\x1B[1mPlease confirm If its Migration from ICM to CP4BA BAW (Yes/No, default: No): \x1B[0m"
    #read -rp "" MIG_ANS
    ...
    then  
        ...
        error "Migration Property File not Found: \"${CASE_MIGRATION_PROPERTY_FILE}\", Please rerun the cp4a-prequisites.sh to create the file"   
        exit 1
    fi
