...
function select_private_catalog(){
    ...
    while true; do
        if [[ -z "$CP4BA_AUTO_PRIVATE_CATALOG" ]]; then
            printf "\x1B[1mDo you want to deploy CP4BA using private catalog? (Yes/No, default: No): \x1B[0m"
            read -rp "" ans
        else
            printf "\x1B[1mDo you want to deploy CP4BA using private catalog? (Yes/No, default: No): $CP4BA_AUTO_PRIVATE_CATALOG\x1B[0m\n"
    ...
    if [[ $PRIVATE_CATALOG == "Yes" ]]; then
        ...
        do
            ...
            if [ -z "$project_name" ]; then
                ...
                verify_existing_csv
                # if [[ ( "$PLATFORM_SELECTED" == "OCP" || "$PLATFORM_SELECTED" == "ROKS" ) && "${ALL_NAMESPACE}" == "No" ]]; then
                #   if [[ ($RUNTIME_MODE != "baw-dev") && ($RUNTIME_MODE != "baw") ]] ; then
                #     check_common_services_cm
                #   fi
                # elif [[ "$PLATFORM_SELECTED" == "other" ]]; then
                #     if [[ ( ($RUNTIME_MODE == "process-flow-dev") || ($RUNTIME_MODE == "process-flow") ) && "${ALL_NAMESPACE}" == "No" ]] ; then
                #       check_common_services_cm
                #     fi
                # fi
                create_project
...
function prepare_olm_install() {
    ...
    # Install prerequisite for Cloud Pak 3.0
    # Import upgrade prerequisite.sh script
    # source ${CUR_DIR}/helper/upgrade/prerequisite.sh
    # if [[ ($RUNTIME_MODE != "baw-dev") && ($RUNTIME_MODE != "baw") ]]; then
    #     info "Starting to install the IBM Cert Manager Operator"
    #     install_ibm_cert_manager "Automatic"
    #     info "Completed install the IBM Cert Manager Operator"

    #     info "Starting to install the IBM Licensing Operator for Cloud Pak 3.0"
    #     install_ibm_license_operator $project_name "Automatic"
    #     info "Completed install the IBM Licensing Operator for Cloud Pak 3.0"
    # fi
...
function select_deployment_type(){
    ...
    if [ -z "$CP4BA_AUTO_DEPLOYMENT_TYPE" ]; then
        ...
        then
            ...
            if [[ "$RUNTIME_MODE" == "baw" || $RUNTIME_MODE == "baw-dev" || $RUNTIME_MODE == "process-flow-dev" || $PRIVATE_CATALOG == "Yes" ]]
            then
              options=("Production")
